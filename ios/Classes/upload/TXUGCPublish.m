...
-(int) publishVideo:(TXPublishParam*)param result:(FlutterResult)fResult{
    if (_publishing == YES) {
    ...
    [_tvcClient uploadVideo:_tvcParam result:^(TVCUploadResponse *resp) {
        ...
        if (resp) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.delegate && [self.delegate respondsToSelector:@selector(onPublishComplete:result:)]) {
                    [self.delegate onPublishComplete:result result:fResult];
                }
            });
        }
        self.publishing = NO;
